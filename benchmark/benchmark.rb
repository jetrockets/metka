...
#
# Metka appears three times in the cloud/write suites, once per tag-cloud
# strategy: bare (no aggregate maintained), materialized_view (statement
# triggers refresh a matview), and table (statement triggers upsert per-tag
# deltas into a summary table). The strategy DDL matches the output of the
# metka:strategies:materialized_view and metka:strategies:table generators.
#
# Usage:
...
ActiveRecord::Schema.define do
  ...
  # force: :cascade — the strategy matview from a previous run depends on its
  # source table and would otherwise block the drop.
  drop_table :metka_mv_posts, if_exists: true, force: :cascade
  ...
  create_table :metka_mv_posts do |t|
    t.string :title
    t.string :tags, array: true
  end
  add_index :metka_mv_posts, :tags, using: :gin
...
# Tag-cloud strategies for the two extra Metka tables, as generated by
# `rails g metka:strategies:materialized_view` and `rails g
# metka:strategies:table` (comments stripped, names bound to the bench tables).
ActiveRecord::Base.connection.execute(<<~SQL)
  CREATE OR REPLACE FUNCTION metka_refresh_tagged_metka_mv_posts_materialized_view() RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
      IF TG_OP = 'INSERT' THEN
        IF NOT EXISTS (SELECT FROM new_rows WHERE tags IS NOT NULL) THEN
          RETURN NULL;
        END IF;
      ELSIF TG_OP = 'UPDATE' THEN
        IF NOT EXISTS (
          (SELECT tags FROM new_rows) EXCEPT ALL (SELECT tags FROM old_rows)
        ) THEN
          RETURN NULL;
        END IF;
      ELSIF TG_OP = 'DELETE' THEN
        IF NOT EXISTS (SELECT FROM old_rows WHERE tags IS NOT NULL) THEN
          RETURN NULL;
        END IF;
      END IF;

      REFRESH MATERIALIZED VIEW CONCURRENTLY tagged_metka_mv_posts;
      RETURN NULL;
    END $$;

  DROP MATERIALIZED VIEW IF EXISTS tagged_metka_mv_posts;
  CREATE MATERIALIZED VIEW tagged_metka_mv_posts AS
    SELECT tag_name, COUNT(*) AS taggings_count
    FROM (SELECT UNNEST(tags) AS tag_name FROM metka_mv_posts) subquery
    GROUP BY tag_name;

  CREATE UNIQUE INDEX idx_metka_mv_posts_tags ON tagged_metka_mv_posts(tag_name);

  CREATE TRIGGER metka_ins_on_metka_mv_posts_tags
  AFTER INSERT ON metka_mv_posts
  REFERENCING NEW TABLE AS new_rows
  FOR EACH STATEMENT
  EXECUTE PROCEDURE metka_refresh_tagged_metka_mv_posts_materialized_view();

  CREATE TRIGGER metka_upd_on_metka_mv_posts_tags
  AFTER UPDATE ON metka_mv_posts
  REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows
  FOR EACH STATEMENT
  EXECUTE PROCEDURE metka_refresh_tagged_metka_mv_posts_materialized_view();

  CREATE TRIGGER metka_del_on_metka_mv_posts_tags
  AFTER DELETE ON metka_mv_posts
...
class MetkaMvPost < ActiveRecord::Base
  self.table_name = "metka_mv_posts"
  include Metka::Model(column: "tags")
end
...
class TaggedMetkaMvPost < ActiveRecord::Base
  self.table_name = "tagged_metka_mv_posts"
end
...
seed_times["metka (materialized_view)"] = Benchmark.realtime { seed_with_insert_all(MetkaMvPost) }
...
sizes = {
  ...
  "metka (materialized_view)" => relation_size("metka_mv_posts", "tagged_metka_mv_posts"),
...
Benchmark.ips do |x|
  ...
  x.report("metka (materialized_view)") { TaggedMetkaMvPost.pluck(:tag_name, :taggings_count) }
...
Benchmark.ips do |x|
  ...
  x.report("metka (materialized_view)") do
    p = MetkaMvPost.new(title: "bench")
    p.tag_list = next_create_set
    p.save!
  end
...
update_ids = {
  ...
  metka_mv: MetkaMvPost.limit(1_000).pluck(:id),
...
Benchmark.ips do |x|
  ...
  x.report("metka (materialized_view)") do
    p = MetkaMvPost.find(next_id(update_ids[:metka_mv]))
    p.tag_list = next_create_set
    p.save!
  end
  x.report("metka (table)") do
...
puts "  materialized_view mismatching tags: #{cloud_mismatches('metka_mv_posts', 'tagged_metka_mv_posts')}"
