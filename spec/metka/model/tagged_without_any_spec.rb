...
RSpec.describe Metka::Model, :db do
  ...
  context 'when use AND as join operator' do
    it 'should return collection without tag ruby' do
      view_posts = ViewPost.tagged_without_any('ruby, wood, foo', join_operator: 'and')

      expect(view_posts.size).to eq(1)
    end

    it 'should return collection without tag php' do
      view_posts = ViewPost.tagged_without_any('php', join_operator: 'and')

      expect(view_posts.size).to eq(3)
    end

    it 'should return collection' do
      expect(ViewPost.tagged_without_any('ruby, crystal, wood', join_operator: 'and').size).to eq(1)
    end
  end
end
